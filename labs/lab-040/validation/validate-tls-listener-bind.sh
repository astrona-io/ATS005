#!/usr/bin/env bash
# Confirms slapd serves dc=example,dc=com, listens on 389 and 636,
# negotiates TLS on both, and accepts the admin bind with the expected rootPW.
...
if ! openssl s_client -connect 127.0.0.1:636 </dev/null >/tmp/lab040-tls-check.log 2>&1; then
  echo "FAIL: TLS handshake against 127.0.0.1:636 did not complete"
  cat /tmp/lab040-tls-check.log
  exit 1
fi

if ! ldapsearch -x -ZZ -H ldap://127.0.0.1 -b "" -s base >/dev/null 2>/tmp/lab040-starttls-check.log; then
  echo "FAIL: ldapsearch -ZZ against ldap://127.0.0.1 (StartTLS required) did not succeed"
  cat /tmp/lab040-starttls-check.log
  exit 1
fi

if ! ldapwhoami -x -D "$ADMIN_DN" -w "$ADMIN_PASSWORD" -ZZ -H ldap://127.0.0.1 >/tmp/lab040-whoami-check.log 2>&1; then
  echo "FAIL: admin bind as $ADMIN_DN with the expected rootPW over StartTLS did not succeed"
  cat /tmp/lab040-whoami-check.log
  exit 1
fi

rm -f /tmp/lab040-tls-check.log /tmp/lab040-starttls-check.log /tmp/lab040-whoami-check.log
